...
import { getCurrentDateAsString } from '@/services/inventory';
...
export const starterSkins = [
  ...
  // Fill with more varieties (rest of the 70 skins)
  ...
  { id: "ssg_dragonfire", name: "Dragonfire", weapon: "SSG 08", image: "https://steamcdn-a.akamaihd.net/apps/730/icons/econ/default_generated/weapon_ssg08_cu_ssg08_dragonfire_scope_light_large.482858c9d088454b42e53f02d927a8203c69e0f3.png", rarity: "Covert", price: 21.16, minFloat: 0, maxFloat: 0.5 },
  { id: "p90_trigon", name: "Trigon", weapon: "P90", image: "https://steamcdn-a.akamaihd.net/apps/730/icons/econ/default_generated/weapon_p90_cu_p90_trigon_light_large.1cae7140807ccd16ff27b6d6a5307373281c1d9f.png", rarity: "Classified", price: 2.52, minFloat: 0.08, maxFloat: 0.76 },
  { id: "aug_chameleon", name: "Chameleon", weapon: "AUG", image: "https://steamcdn-a.akamaihd.net/apps/730/icons/econ/default_generated/weapon_aug_cu_aug_chameleonaire_light_large.83f1e867b18d4be0dead16921eb06e14e3700f10.png", rarity: "Covert", price: 1.75, minFloat: 0, maxFloat: 0.5 },
  { id: "nova_hyper", name: "Hyper Beast", weapon: "Nova", image: "https://steamcdn-a.akamaihd.net/apps/730/icons/econ/default_generated/weapon_nova_cu_nova_hyperbeast_light_large.605ea58d77d6eb95ede2f0ffddc8b20ca75af4b1.png", rarity: "Classified", price: 1.69, minFloat: 0, maxFloat: 1 }
];

// Function to populate a user's inventory with starter skins
export const populateStarterInventory = async (userId: string): Promise<boolean> => {
  try {
    console.log("Starting inventory population for user:", userId);
    
    // Generate 70 starter inventory items
    const inventoryItems = starterSkins.map(skin => {
      return generateStarterItem(
        skin.id,
        skin.name,
        skin.weapon,
        skin.image,
    ...
    if (profile && profile.inventory_populated) {
      console.log("User inventory already populated, skipping");
      return true;
    }
    ...
    const { error: insertError } = await supabase
      .from('inventory')
      .insert(inventoryItems.map(item => ({
    ...
    if (insertError) {
      console.error("Error populating inventory:", insertError);
      return false;
    }
    ...
    console.log(`Successfully populated inventory with ${inventoryItems.length} skins for user:`, userId);
    return true;
  } catch (error) {
    console.error("Error in populateStarterInventory:", error);
    return false;
  }
};
